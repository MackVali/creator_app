...
export async function getUserStats(cookieStore = nextCookies()): Promise<UserStats> {
...
export async function getMonumentsSummary(cookieStore = nextCookies()): Promise<MonumentCounts> {
  const supabase = getSupabaseServer(cookieStore as any)
  const { data, error } = await supabase
    .from('monuments_summary_v')
    .select('category,count')
  const base: MonumentCounts = { Achievement: 0, Legacy: 0, Triumph: 0, Pinnacle: 0 }
  if (error || !data) return base
  for (const row of data as { category: keyof MonumentCounts; count: number }[]) {
    if (row?.category in base) base[row.category] = row.count ?? 0
  }
  return base
}

export async function getSkillsAndGoals(cookieStore = nextCookies()): Promise<{ skills: SkillItem[]; goals: string[] }> {
  const supabase = getSupabaseServer(cookieStore as any)
