...
const TIME_BLOCK_FLAME_LEVELS = new Set<FlameLevel>([
  "NO",
  "LOW",
  "MEDIUM",
  "HIGH",
...
function normalizeTimeBlockEnergy(value?: string | null): FlameLevel {
  const normalized = value?.trim().toUpperCase();
  return TIME_BLOCK_FLAME_LEVELS.has(normalized as FlameLevel)
    ...
    .map((row) => {
      ...
      return {
        ...
        name:
          ...
          "Time Block",
        detail,
        energy: normalizeTimeBlockEnergy(row.energy),
...
export async function loadPriorityEditorProps(
  supabase: NonNullable<ReturnType<typeof getSupabaseServer>>,
  userId: string
): Promise<PriorityEditorClientProps> {
