...
export default function MonumentNoteDatabasePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const fetchedNote = await getMonumentNote(monumentId, noteId);
        if (!isMounted) return;
        ...
        console.error("Failed to load monument note database", {
          error,
          monumentId,
          noteId,
          databaseId,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSaving(true);
      try {
        const saved = await updateMonumentNote(monumentId, note.id, {
        ...
        console.error("Failed to save monument note database", {
          error,
          monumentId,
          noteId,
          databaseId,
  ...
  }, [databaseId, isLoading, isSaving, lastSavedSnapshot, monumentId, note, noteId, noteMetadata]);

  function handleBack() {
    router.push(`/monuments/${monumentId}/notes/${noteId}`);
  }
  ...
  async function handleDeleteDatabase() {
    ...
    try {
      ...
      console.error("Failed to delete monument note database", {
        error,
        monumentId,
        noteId,
        databaseId,
