...
export default function SkillRow({
  ...
  const level = progress?.level ?? skill.level ?? 1;
  const prestige = progress?.prestige ?? 0;
  const badges = progress?.badges ?? [];
  const showLevel = level !== null && level !== undefined;
  // Always show progress bar for consistency, even with default values
        ...
        <div className="flex min-w-[24%] flex-col gap-1">
          ...
          >
            <div
              ...
              style={{
                width: computeWidth(progress?.progressPercent ?? 0),
