...
function isScrollContainerAtTop(
...
export function SkillDetail({
  ...
  const handlePullExitStart = useCallback(
    (event: PointerEvent<HTMLElement>) => {
      if (
        ...
        !isScrollContainerAtTop(scrollContainerRef) ||
    ...
    [pullExitBlocked, resetPullExit, scrollContainerRef]
  );
  ...
  const handlePullExitMove = useCallback(
    ...
    [handleCloseOrBack, pullExitBlocked, scrollContainerRef]
  );
  ...
  const errorMainClassName = clsx(
    "relative px-4",
    onClose
      ? "pb-[calc(4rem+env(safe-area-inset-bottom,0px))] pt-4"
      : "pb-16 pt-10"
  );
