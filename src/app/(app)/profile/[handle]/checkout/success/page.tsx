...
export default async function ProfileCheckoutSuccessPage({ params, searchParams }: CheckoutReturnPageProps) {
  const { handle } = params;
  ...
  const order =
    checkoutId && sessionId && normalizedHandle
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-8 px-4 py-12 lg:px-0">
      <section className="rounded-3xl border border-white/10 bg-white/5 p-8 text-white shadow-[0_30px_60px_rgba(15,23,42,0.65)]">
        ...
        <h1 className="mt-3 text-3xl font-semibold">Thanks for supporting this creator.</h1>
        <p className="mt-3 text-base text-white/70">
          Stripe redirected you back after payment. This receipt reflects the persisted product order record.
        </p>
