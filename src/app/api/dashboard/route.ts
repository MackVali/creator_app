...
export async function GET() {
  ...
  const [{ data: stats }, { data: monuments }, { data: skills }, { data: goals }] = await Promise.all([
    supabase.from('user_stats_v').select('level,xp_current,xp_max').maybeSingle(),
  ...
  const statsOut: UserStats = stats ?? { level: 1, xp_current: 0, xp_max: 4000 }
  const mBase: MonumentCounts = { Achievement: 0, Legacy: 0, Triumph: 0, Pinnacle: 0 }
  for (const m of (monuments ?? []) as { category: keyof MonumentCounts; count: number }[]) {
  ...
  return NextResponse.json({ stats: statsOut, monuments: mBase, skills: skillsOut, goals: goalsOut })
}
