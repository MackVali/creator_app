...
export function SharedMealPlanPanel({
  ...
  const [groceryPickerError, setGroceryPickerError] = useState<string | null>(null);
  const [pendingChoice, setPendingChoice] = useState<string | null>(null);
  const [targetSetupOpen, setTargetSetupOpen] = useState(false);
  const mealTemplates = useNutritionMealTemplates(50, !targetSetupOpen);
  const handleTargetSetupOpenChange = useCallback((open: boolean) => {
    setTargetSetupOpen(open);
    onNutritionTargetSetupOpenChange?.(open);
  }, [onNutritionTargetSetupOpenChange]);

  useEffect(() => {
    if (surface !== "grocery") {
      setGroceries([]);
      setGroceryPickerError(null);
      return;
    }
    ...
    fetch("/api/food-resources?status=active&limit=200", { signal: controller.signal })
      .then((response) => response.json().then((payload) => response.ok ? payload.foodResources ?? [] : Promise.reject(new Error(payload.error))))
      .then((groceryRows) => {
      ...
      .catch((reason) => {
        if (!controller.signal.aborted) {
          setGroceryPickerError(reason instanceof Error ? reason.message : "Grocery choices couldn’t load.");
        }
      });
  ...
  return <section className={targetSetupOpen && surface === "nutrition" ? "contents" : "mt-3 overflow-hidden rounded-2xl border border-white/[0.07] bg-[#090909]"} aria-label="Meal Plan">
    ...
    {!isLoading && !error && plan ? <div>
      {isRefreshing ? <div className="flex items-center gap-2 border-b border-white/[0.045] px-3 py-2 text-[11px] text-white/34"><LoaderCircle className="h-3.5 w-3.5 animate-spin" /> Refreshing plan...</div> : null}
      {backgroundError ? <div className="border-b border-red-200/10 px-3 py-2 text-[11px] text-red-100/70">{backgroundError}</div> : null}
