...
export async function deleteMonumentWithReassignment({
  ...
  if (uniqueSkillIds.length > 0) {
    const { data: existingUncategorizedSkillLinks, error: existingLinksError } = await supabase
      .from("monument_skills")
      .select("skill_id")
      .eq("user_id", userId)
      .eq("monument_id", uncategorizedId)
    ...
    const alreadyLinkedSkillIds = new Set(
      (existingUncategorizedSkillLinks ?? [])
        .map((row) => row.skill_id)
        .filter(Boolean) as string[],
    );

    const linksToInsert = uniqueSkillIds
      .filter((skillId) => !alreadyLinkedSkillIds.has(skillId))
      .map((skillId) => ({
        user_id: userId,
