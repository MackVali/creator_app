...
export async function fetchPriorityEnergyLookups(client: Client): Promise<PriorityEnergyLookups> {
  const now = Date.now();
  ...
  if (priorityRes.error) {
    ...
    for (const row of (priorityRes.data ?? []) as { id?: number | null; name?: string | null }[]) {
  ...
  if (energyRes.error) {
    ...
    for (const row of (energyRes.data ?? []) as { id?: number | null; name?: string | null }[]) {
      if (row?.id == null || typeof row.name !== 'string') continue;
...
export async function fetchReadyTasks(client?: Client): Promise<TaskLite[]> {
  ...
  return ((data ?? []) as TaskRecord[]).map(record => {
    const priorityName = resolvePriorityLabel(record.priority, lookups.priority);
