...
async function persistHabitPlacements(params: {
  ...
  onChunk?: (details: { inserted: number; failures: number; remaining: number }) => void
  ...
  for (let index = 0; index < placements.length; index += HABIT_PERSIST_BATCH_SIZE) {
    ...
    let attempt = 0
    let chunkInserted: ScheduleInstance[] | null = null

    while (attempt < 2 && !chunkInserted) {
      attempt += 1
      const response = await createInstancesBatch(
        supabase,
        chunk.map(placement => ({
          userId,
      ...
      if (!response.error && response.data) {
        chunkInserted = response.data as ScheduleInstance[]
        break
      }
