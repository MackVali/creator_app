        ...
        if (placed.data) {
          ...
          if (placementWindow?.key) {
            ...
            if (existingBounds) {
              ...
              if (
                existingBounds.front.getTime() > existingBounds.back.getTime()
              ) {
...
async function scheduleHabitsForDay(params: {
  ...
  const findFirstSyncConflict = (
    ...
    segments.find(
      (segment) => endMs > segment.start && startMs < segment.end
    ) ?? null;
  ...
  while (habitQueue.length > 0) {
    ...
    if (!needsUpdate && existingInstance) {
      const overlapsExisting = dayInstances.some((inst) => {
        ...
        if (!Number.isFinite(instStartMs) || !Number.isFinite(instEndMs))
          return false;
...
const attemptProjectConflictPlacement = async (
  ...
  const existingInstances = options.dayInstances.filter(
    (inst) => inst.id !== conflict.id
  );
  const placed = await placeItemInWindows({
    userId: options.userId,
    item,
  ...
  const decision: ScheduleDraftPlacement["decision"] = conflict.id
    ? "rescheduled"
    : "new";
  result.timeline.push({
    type: "PROJECT",
  ...
  const existingIndex = options.dayInstances.findIndex(
    (inst) => inst.id === conflict.id
  );
  if (existingIndex >= 0) {
