...
export async function getGoalsForUser(userId: string): Promise<Goal[]> {
  ...
  const { data, error } = await supabase
    .from("goals")
    .select(
      "id, name, emoji, priority, energy, priority_code, energy_code, why, created_at, active, status, monument_id, circle_id, area_id, roadmap_id, weight, weight_boost, due_date, global_rank, updated_at, monument:monuments(emoji)"
    )
    .eq("user_id", userId)
...
export async function getGoalById(goalId: string): Promise<Goal | null> {
  ...
  const { data, error } = await supabase
    .from("goals")
    .select(
      "id, name, emoji, priority, energy, priority_code, energy_code, why, created_at, active, status, monument_id, circle_id, area_id, roadmap_id, due_date, global_rank, updated_at, monument:monuments(emoji)"
    )
    .eq("id", goalId)
    .single();
