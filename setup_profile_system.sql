...
CREATE INDEX IF NOT EXISTS idx_content_cards_user_order ON public.content_cards(user_id, position);
CREATE INDEX IF NOT EXISTS idx_content_cards_user_active ON public.content_cards(user_id, is_active);
CREATE INDEX IF NOT EXISTS idx_profiles_username_lookup ON public.profiles(lower(username));
...
DROP TRIGGER IF EXISTS content_cards_set_updated_at ON public.content_cards;
...
-- 10. RLS Policies for social_links
...
-- 11. RLS Policies for content_cards
...
GRANT ALL ON public.profile_themes TO authenticated;

-- 13. Add RLS policy to profiles table for verified status (admin only)
DROP POLICY IF EXISTS "profiles admin verified" ON public.profiles;
CREATE POLICY "profiles admin verified" ON public.profiles
