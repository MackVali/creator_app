...
import { finalizeTodaySimple } from '../../../lib/scheduler/schedule_eod.ts'
...
serve(async req => {
  try {
    ...
    const updated = await finalizeTodaySimple(userId, dateLocal)
    return jsonResponse({
      userId,
      date: dateLocal.toISOString(),
      finalizedCount: updated.length,
      instanceIds: updated.map(item => item.id),
    })
  } catch (error) {
    console.error('finalize_today_simple failure', error)
