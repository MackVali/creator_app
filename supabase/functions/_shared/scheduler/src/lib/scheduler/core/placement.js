...
export async function placeItemInWindows(params) {
    ...
    for (const [index, w] of windows.entries()) {
        ...
        const startMs = typeof notBeforeMs === 'number' ? Math.max(windowStartMs, notBeforeMs) : windowStartMs;
        const rangeStart = new Date(startMs);
        const { data: taken, error } = await fetchInstancesForRange(client, userId, rangeStart.toISOString(), windowEnd.toISOString());
        if (error) {
            return { error };
        }
        const filtered = (taken ?? []).filter(inst => {
            if (inst.id === reuseInstanceId)
                return false;
        ...
        const sorted = filtered.sort((a, b) => new Date(a.start_utc).getTime() - new Date(b.start_utc).getTime());
        ...
        for (const block of sorted) {
            ...
            const effectiveBlockStartMs = typeof notBeforeMs === 'number'
