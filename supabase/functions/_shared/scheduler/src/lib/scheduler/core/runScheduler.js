import { fetchBacklogNeedingSchedule, fetchInstancesForRange, createInstance, createInstancesBatch, } from './instanceRepo';
...
export async function scheduleBacklog(client, userId, baseDate = new Date(), options) {
    ...
    try {
        ...
        if (habitPlacements.length > 0) {
            emitProgress({
                type: 'habits-persisting',
                total: habitPlacements.length,
                batchSize: HABIT_PERSIST_BATCH_SIZE,
            });
            const persistence = await persistHabitPlacements({
                supabase,
                userId,
                placements: habitPlacements,
                onChunk: info => emitProgress({
                    type: 'habit-persistence-chunk',
...
const HABIT_PERSIST_BATCH_SIZE = 50;
async function persistHabitPlacements(params) {
    const { supabase, userId, placements, onChunk } = params;
    const inserted = [];
    const failures = [];
    await ensureHabitSourceTypeReady(supabase);
    for (let index = 0; index < placements.length; index += HABIT_PERSIST_BATCH_SIZE) {
        const chunk = placements.slice(index, index + HABIT_PERSIST_BATCH_SIZE);
        let attempt = 0;
        let chunkInserted = null;
        while (attempt < 2 && !chunkInserted) {
            attempt += 1;
            const response = await createInstancesBatch(supabase, chunk.map(placement => ({
                userId,
                sourceId: placement.habit.id,
                sourceType: 'HABIT',
                windowId: placement.habit.windowId,
                startUTC: placement.habit.startUTC,
                endUTC: placement.habit.endUTC,
                durationMin: placement.habit.durationMin,
                weightSnapshot: 0,
                energyResolved: placement.habit.energyResolved?.toUpperCase() ?? 'NO',
            })));
            if (!response.error && response.data) {
                chunkInserted = response.data;
                break;
            }
            if (isHabitSourceTypeError(response.error) && attempt === 1) {
        ...
        if (chunkInserted) {
            ...
            if (onChunk) {
                const remaining = Math.max(placements.length - (index + chunk.length), 0);
                onChunk({ inserted: chunkInserted.length, failures: 0, remaining });
            }
            continue;
        }
        const chunkFailures = await persistHabitPlacementsIndividually({
            supabase,
            userId,
            placements: chunk,
        });
        inserted.push(...chunkFailures.inserted);
        failures.push(...chunkFailures.failures);
        if (onChunk) {
            const remaining = Math.max(placements.length - (index + chunk.length), 0);
            onChunk({
                inserted: chunkFailures.inserted.length,
                failures: chunkFailures.failures.length,
                remaining,
            });
        }
    }
...
async function persistHabitPlacementsIndividually(params) {
