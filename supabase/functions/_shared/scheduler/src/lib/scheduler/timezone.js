...
function makeZonedDate(input, timeZone) {
    const { year, month, day, hour, minute, second = 0, millisecond = 0, } = input;
...
export function startOfDayInTimeZone(date, timeZone) {
    const parts = getDateTimeParts(date, timeZone);
    return makeZonedDate({ year: parts.year, month: parts.month, day: parts.day, hour: 0, minute: 0, second: 0 }, timeZone);
}
export function addDaysInTimeZone(date, amount, timeZone) {
    if (amount === 0)
        return new Date(date);
    const parts = getDateTimeParts(date, timeZone);
    const noon = makeZonedDate({ year: parts.year, month: parts.month, day: parts.day, hour: 12, minute: 0, second: 0 }, timeZone);
...
export function addMonthsInTimeZone(date, amount, timeZone) {
    if (amount === 0) {
    ...
    return makeDateInTimeZone({ year: targetYear, month: targetMonth, day: targetDay, hour: 0, minute: 0 }, timeZone);
}
export function setTimeInTimeZone(date, timeZone, hours, minutes) {
    const parts = getDateTimeParts(date, timeZone);
...
export function differenceInCalendarDaysInTimeZone(base, target, timeZone) {
    const baseParts = getDateTimeParts(base, timeZone);
    const targetParts = getDateTimeParts(target, timeZone);
    const baseMid = makeZonedDate({ year: baseParts.year, month: baseParts.month, day: baseParts.day, hour: 12, minute: 0, second: 0 }, timeZone);
    const targetMid = makeZonedDate({ year: targetParts.year, month: targetParts.month, day: targetParts.day, hour: 12, minute: 0, second: 0 }, timeZone);
    const diffMs = targetMid.getTime() - baseMid.getTime();
    return Math.round(diffMs / (24 * 60 * 60 * 1000));
}
export function differenceInCalendarMonthsInTimeZone(base, target, timeZone) {
...
export function makeDateInTimeZone(input, timeZone) {
