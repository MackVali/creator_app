-- Hosted connector support: ensure OAuth columns exist and policy allows OAuth integrations
...
-- Backfill missing API key header values for legacy rows
UPDATE public.source_integrations
SET auth_header = 'X-API-Key'
WHERE auth_mode = 'api_key'
  AND (auth_header IS NULL OR length(trim(auth_header)) = 0);

-- Table for short-lived OAuth state during hosted connector flows
