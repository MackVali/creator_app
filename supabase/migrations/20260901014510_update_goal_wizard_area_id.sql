-- Persist Area ownership from the goal wizard RPC while preserving optional monument compatibility.
...
begin
  ...
  insert into public.goals (user_id, name, priority, energy, area_id, monument_id, why, due_date)
  values (
    goal_user_id,
    ...
    coalesce(
      nullif(payload->>'area_id', ''),
      nullif(payload->>'areaId', '')
    ),
    coalesce(
