-- Ensures goal.status is always recalculated with the correct completion rule.
-- Treat a project as complete if completed_at is set or the stage enum is RELEASE.
-- Do not auto-complete goals with zero projects.
...
create trigger trg_projects_goal_status_sync_update
  after update of completed_at, stage, goal_id on public.projects
  for each row execute function public.trigger_sync_goal_status_for_project();

-- Backfill every goal to align with the corrected completion rule.
with goal_stats as (
  select
    g.id,
