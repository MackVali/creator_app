-- Migration to ensure proper RLS policies for cats and skills tables
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='cats' AND policyname='cats_select_own') THEN
    CREATE POLICY cats_select_own ON public.cats FOR SELECT USING (user_id = auth.uid());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='cats' AND policyname='cats_insert_own') THEN
    CREATE POLICY cats_insert_own ON public.cats FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='cats' AND policyname='cats_update_own') THEN
    CREATE POLICY cats_update_own ON public.cats FOR UPDATE USING (user_id = auth.uid());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='cats' AND policyname='cats_delete_own') THEN
    CREATE POLICY cats_delete_own ON public.cats FOR DELETE USING (user_id = auth.uid());
  END IF;
END $$;
